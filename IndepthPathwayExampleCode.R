...
up.assoc <- pathwayAssociation(disambiguatePathway=up.disambiguate,  compare.list, feature.list, preCalmatrix,selectn=30, minsize=10)
saveRDS(up.assoc, file="up.assoc_BySCDE_quiescentVsActive.rds")
down.assoc <- pathwayAssociation(disambiguatePathway=down.disambiguate, compare.list,feature.list,preCalmatrix,minsize=10)
...
selectn=30
pathway.merge=mergePathway(up.pathway=up.disambiguate[[1]][1:min(c(selectn,nrow(up.disambiguate[[1]]))),],down.pathway=down.disambiguate[[1]][1:min(c(selectn,nrow(down.disambiguate[[1]]))),])
pathway.merge.assoc <- pathwayAssociation(disambiguatePathway=list(pathway.merge), compare.list,feature.list,preCalmatrix,minsize=10)
...
PlotGenesetMatrix(genomatrix=expData,geneset=compare.list[[Pathway]],weights=weight,annCol=sampleClass)
graphics.off()
...
disambiguate_CSEA_Up<-disambiguationCSEA(GEA.result=CSEA.result_Up,uniConSig.result=uniConSig_Up,compare.list=compare.list,topn=min(c(topn,nrow(CSEA.result_Up))),
                                  p.cut=0.01,p.adjust.method="bonferroni")#p.adjust.method-- chose one: NULL or "bonferroni" or "BH"
saveRDS(disambiguate_CSEA_Up, file="up.CSEA_disambiguate_BySCDE_quiescentVsActive.rds")

topn=100 #specify the number of top pathways to disambiguate
disambiguate_CSEA_Dw<-disambiguationCSEA(GEA.result=CSEA.result_Dw,uniConSig.result=uniConSig_Dw,compare.list=compare.list,topn=min(c(topn,nrow(CSEA.result_Dw))),
                                        p.cut=0.01,p.adjust.method="bonferroni")#p.adjust.method-- chose one: NULL or "bonferroni" or "BH"
saveRDS(disambiguate_CSEA_Dw, file="Dw.CSEA_disambiguate_BySCDE_quiescentVsActive.rds")
...
CSEA_up.assoc <- pathwayAssociation(disambiguatePathway=disambiguate_CSEA_Up,  compare.list, feature.list, preCalmatrix,selectn=30, minsize=10)
saveRDS(CSEA_up.assoc, file="CSEA_up.assoc_BySCDE_quiescentVsActive.rds")
CSEA_down.assoc <- pathwayAssociation(disambiguatePathway=disambiguate_CSEA_Dw, compare.list,feature.list,preCalmatrix,selectn=30, minsize=10)
...
draw.network(pathway.out=disambiguate_CSEA_Up[[1]],assoc=CSEA_up.assoc,NES.cut=2,node.size=1,line.thickness=1,font.size=0.4,wrap.string.width=15)
graphics.off()
...
#disambiguate up and downregulated pathways
topn=30 #specify the number of top pathways to disambiguate
up.disambiguate<-disambiguationGSEA(GEA.result=up.GSEA.result,weight=weight,compare.list=compare.list,topn=min(c(topn,nrow(up.GSEA.result))),
                                     p.cut=0.05,p.adjust.method="bonferroni")#p.adjust.method-- chose one: NULL or "bonferroni" or "BH"
saveRDS(up.disambiguate, file="Up.GSEA_disambiguate_BySCDE_quiescentVsActive.rds")


down.disambiguate<-disambiguationGSEA(GEA.result=down.GSEA.result,weight=-weight,compare.list=compare.list,topn=min(c(topn,nrow(down.GSEA.result))),
                                       p.cut=0.05,p.adjust.method="bonferroni")
saveRDS(down.disambiguate, file="Dw.GSEA_disambiguate_BySCDE_quiescentVsActive.rds")
