...
## Simulating multifaceted variability of scRNAseq data based on Synthetic model of multiple variability factors for Simulation (SymSim)

```
## This example R code is used in our study to simulate the multifaceted variability of scRNAseq data
## Example R code is available, "SimulationSingleCellRNAseq_Example.R"   Detailed explanation is available at https://rpubs.com/zhenmiao/658664  https://github.com/YosefLab/SymSim
## To install SymSim
LoadPackage_SymSim()
#library("devtools")
...
ExpDataProc <- ExpData %>% dplyr::select(-MGI.symbol) %>% dplyr::filter(!duplicated(HGNC.symbol)) %>% tibble::column_to_rownames("HGNC.symbol"); ExpDataProc[1:2,1:3]
